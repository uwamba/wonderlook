...
if($amount>2000000){
     ...
     $_SESSION['error']='Please note that total amount was RWF '.$new_amount.' The 2M wilbe paid with momo and the rest ('.$rest.')will be paid on Delivery by clicking on continue you accept the condition for any question call: +250788308398' ; 
      header('location: momo_pay_confirmation.php'); 
   }
else{
   $_SESSION['amount_paid']=$amount;
   $_SESSION['rest']=0;  
...
try {
  ...
  if ($response_token->getStatus() == 200) {
    ...
    if(isset($reference_key)){
      ...
      $reference_key=generatekey();
      $_SESSION['reference_key']=$reference_key;
      $request->setHeader(array(
      ...
      try {
        ...
        if ($response->getStatus() == 202 || $response->getStatus() == 202) {
          ...
          $_SESSION['success']="The payment request have been initiated. Please check on your phone and confirm or dial *182*7*1# confirm your payemnt and  then after click on continue";
