              ...
              <table id="example1" class="table table-bordered">
                ...
                <tbody>
                  <?php
                    $conn = $pdo->open();
                    try{
                        $stmt = $conn->prepare("SELECT *, shipping.id AS shipping_id, shipping.status AS shipping_status FROM shipping LEFT JOIN users ON shipping.user_id=users.id");
                      $stmt->execute();
                      foreach($stmt as $row){
...
function getRow(id){
  $.ajax({
    ...
    success: function(response){
      ...
      $('.email').val("uwambadodo@gmail.com");
